...
public class Solution {
    ...
    public TreeNode deserialize(String data) {
        ...
        while (!q.isEmpty()) {
            ...
            commaIndex = s.indexOf(",");  // always 1
            str = s.substring(0, commaIndex); // include 0 idx, but not last
            s.delete(0, commaIndex + 1);  // starting se sab delete bss last point ko rakhna hai
            // If the value is not "#", create a new
            // left child and push it to the queue
            if (!str.equals("#")) {
                TreeNode leftNode = new TreeNode(Integer.parseInt(str));  // ye =  s.delete() hone ke baad first char
                // ko node bana raha hai aur usko RST bana rahe hai aur queue me daal rahe hai
                node.left = leftNode;
                q.offer(leftNode);
            }
